...
def get_Dense_model(input_shape=9, output_shape=6, num_nodes=200, dropout=0.45, regularization=0.00001, num_layers=4):
    ...
    dense_model = tf.keras.Model(inputs=visible1, outputs=output)
    return dense_model
...
os.chdir('Force Prediction/K-Fold Dense')
...
for fold in range(10):
    ...
    model[fold] = get_Dense_model(num_nodes=1024, num_layers=4)
    ...
    # Show the plot
    plt.savefig(f'Boxplot_MDPE_Fold_{fold}_Dense.png')
    ...
    plt.ylabel('Tz [Nm]')
    plt.xlim([0,360])
    plt.savefig(f'Highest_Error_Cycle_Fold{fold}_Dense.png')
    ...
    plt.savefig(f'Mean_Cycle_Fold{fold}_Dense.png')
    
# Save variables to files
# Save
with open('radius_predictions_Dense.pkl', 'wb') as f:
    pickle.dump(radius_predictions, f)

with open('real_y_Dense.pkl', 'wb') as f:
    pickle.dump(real_y, f)

with open('mdpe_Dense.pkl', 'wb') as f:
    pickle.dump(mdpe, f)

with open('md_column_Dense.pkl', 'wb') as f:
    pickle.dump(md_column, f)

with open('std_column_Dense.pkl', 'wb') as f:
    pickle.dump(std_column, f)

with open('mean_column_Dense.pkl', 'wb') as f:
    pickle.dump(mean_column, f)
    
with open('model_history_Dense.pkl', 'wb') as f:
    pickle.dump(model_history.history, f)

for i, model in enumerate(model):
    model.save(f'model_Dense_fold{i}.h5')
## To load models
# loaded_models = []
# for i in range(len(models)):
#     loaded_models.append(tf.keras.models.load_model(f'model_Dense_concat_{i}.h5'))