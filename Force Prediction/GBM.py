...
for fold, (train_val_index, test_index) in enumerate(kf.split(participants)):
    ...
    for i in range(num_outputs):
        model = xgb.train(param, xgb.DMatrix(train_X, label=train_y.iloc[:, i]), num_round)
        models.append(model)

    # Predict outputs for the entire test set
    radius_predictions[fold] = np.zeros_like(test_y)
    for i, model in enumerate(models):
        radius_predictions[fold][:, i] = model.predict(xgb.DMatrix(test_X))
    real_y[fold] = test_y.to_numpy()
