...
with tf.device('/device:CPU:0'):
    ## Functions
    def get_Resnet_model(num_filters, kernel_size, num_layers):
        # Define the input layer
        input_layer = tf.keras.layers.Input(shape=(None, 9))

        # Add the initial Conv1D layer with L2 regularization
        x = tf.keras.layers.Conv1D(num_filters, kernel_size=kernel_size, padding='same', 
                                kernel_regularizer=regularizers.l2(0.001))(input_layer)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation('relu')(x)

        # Add the residual blocks with L2 regularization
        for i in range(num_layers):
            shortcut = x
            if i % 5 == 0 and i > 0:  # Double the number of filters every 5 layers
                num_filters *= 2
                shortcut = tf.keras.layers.Conv1D(num_filters, kernel_size=1, padding='same')(shortcut)
            y = tf.keras.layers.Conv1D(num_filters, kernel_size=kernel_size, padding='same', 
    ...
    os.chdir('Force Prediction/Resnet')
    ...
    model = get_Resnet_model(num_filters=32, kernel_size=32, num_layers=40)
    print(model.summary())
    tf.keras.utils.plot_model(model, to_file='ResNet_model_architecture.png', show_shapes=True, show_layer_names=True)
    ...
    # Show the plot
    plt.savefig('Boxplot_MDPE_Resnet.png')
    ...
    plt.ylabel('Tz [Nm]')
    plt.xlim([0, 360])
    plt.savefig('Highest_Error_Cycle_Resnet.png')
    ...
    plt.savefig('Mean_Cycle_Resnet.png')

    # Save variables to files
    # Save
    with open('radius_predictions_Resnet.pkl', 'wb') as f:
        pickle.dump(radius_predictions, f)

    with open('real_y_Resnet.pkl', 'wb') as f:
        pickle.dump(real_y, f)

    with open('mdpe_Resnet.pkl', 'wb') as f:
        pickle.dump(mdpe, f)

    with open('md_column_Resnet.pkl', 'wb') as f:
        pickle.dump(md_column, f)

    with open('std_column_Resnet.pkl', 'wb') as f:
        pickle.dump(std_column, f)

    with open('mean_column_Resnet.pkl', 'wb') as f:
    ...
    #     loaded_models.append(tf.keras.models.load_model(f'model_Resnet_concat_{i}.h5'))