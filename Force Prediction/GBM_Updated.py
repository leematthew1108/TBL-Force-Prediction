...
# Separate X and y
train_X = train_data[:, :, :-6].reshape(-1, 9)
train_y = train_data[:, :, -6:].reshape(-1, 6)
...
# Show the plot
plt.savefig('Boxplot_MDPE_GBM.png')
...
plt.ylabel('Tz [Nm]')
plt.xlim([0, 360])
plt.savefig('Highest_Error_Cycle_GBM.png')
...
plt.savefig('Mean_Cycle_GBM.png')

# Save variables to files
# Save
with open('radius_predictions_GBM.pkl', 'wb') as f:
    pickle.dump(radius_predictions, f)

with open('real_y_GBM.pkl', 'wb') as f:
    pickle.dump(real_y, f)

with open('mdpe_GBM.pkl', 'wb') as f:
    pickle.dump(mdpe, f)

with open('md_column_GBM.pkl', 'wb') as f:
    pickle.dump(md_column, f)

with open('std_column_GBM.pkl', 'wb') as f:
    pickle.dump(std_column, f)

with open('mean_column_GBM.pkl', 'wb') as f:
    pickle.dump(mean_column, f)

## To load models
# loaded_models = []
# for i in range(len(models)):
#     loaded_models.append(tf.keras.models.load_model(f'model_GBM_concat_{i}.h5'))