...
def get_tcn_model(num_filters, kernel_size, num_layers):
    ...
    for i in range(num_layers):
        ...
        x = Conv1D(num_filters, kernel_size, dilation_rate=2**i, padding='causal', 
                   kernel_regularizer=regularizers.l1_l2(0.0001))(x)
        x = Activation('relu')(x)
...
os.chdir('Force Prediction/K-Fold TCN')
...
for fold in range(10):
    ...
    model[fold] = get_tcn_model(num_filters=128, kernel_size=64, num_layers=4)
    ...
    # Show the plot
    plt.savefig(f'Boxplot_MDPE_Fold_{fold}_TCN.png')
    ...
    plt.ylabel('Tz [Nm]')
    plt.xlim([0,360])
    plt.savefig(f'Highest_Error_Cycle_Fold{fold}_TCN.png')
    ...
    plt.savefig(f'Mean_Cycle_Fold{fold}_TCN.png')
...
with open('model_history_TCN.pkl', 'wb') as f:
    pickle.dump(model_history.history, f)

for i, model in enumerate(model):
    model.save(f'model_TCN_fold{i}.h5')
## To load models
# loaded_models = []
