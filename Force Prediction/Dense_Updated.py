...
with tf.device('/device:GPU:0'):
    ...
    os.chdir('Force Prediction/Dense')
    ...
    model = get_Dense_model(num_nodes=200, num_layers=1)
    print(model.summary())
    tf.keras.utils.plot_model(model, to_file='Dense_model_architecture.png', show_shapes=True, show_layer_names=True)
    ...
    # Show the plot
    plt.savefig('Boxplot_MDPE_Dense.png')
    ...
    plt.ylabel('Tz [Nm]')
    plt.xlim([0, 360])
    plt.savefig('Highest_Error_Cycle_Dense.png')
    ...
    plt.savefig('Mean_Cycle_Dense.png')
