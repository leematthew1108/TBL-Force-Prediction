...
os.chdir('Force Prediction/K-Fold BiLSTM')
...
for fold in range(10):
    ...
    # Show the plot
    plt.savefig(f'Boxplot_MDPE_Fold_{fold}_BiLSTM.png')
    ...
    plt.ylabel('Tz [Nm]')
    plt.xlim([0,360])
    plt.savefig(f'Highest_Error_Cycle_Fold{fold}_BiLSTM.png')
    ...
    plt.savefig(f'Mean_Cycle_Fold{fold}_BiLSTM.png')
...
with open('model_history_BiLSTM.pkl', 'wb') as f:
    pickle.dump(model_history.history, f)

for i, model in enumerate(model):
    model.save(f'model_BiLSTM_fold{i}.h5')
## To load models
# loaded_models = []
# for i in range(len(models)):
#     loaded_models.append(tf.keras.models.load_model(f'model_BiLSTM_{i}.h5'))