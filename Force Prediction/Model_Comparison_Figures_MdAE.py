...
# plt.xlabel('Feature')
# plt.ylabel('MAE [N]' if 'F' in feature_names[0] else 'MAE [Nm]')
...
# average_performance = {}
# for model in key_order:
#     model_performance = np.mean(error[model])
...
#     model_mean = np.mean(error[model])
#     model_std = np.std(error[model])
...
# feature_values = []
# for model in key_order:
#     error_values = np.mean(error[model])
#     feature_values.append(error_values)
# _, p_value = kruskal(*feature_values)
# model_comparison_results[feature]['p-value'] = p_value
...
#         if j == 0:
#             plt.plot(real_y_data, label='Ground Truth')
        
#         # Plot the radius_prediction data for the current model
#         plt.plot(radius_prediction_data, label=f'({model} Model Prediction)')
        
#         # Set the title and labels for the subplot
#         plt.title(f'{feature}')
#         plt.xlabel('% Propulsion Cycle')
...
# # plt.show()
# # Save the figure
# plt.savefig('./Model_Comparison_Representative_Cycle.png')
...
#         plt.plot(avg_radius_predictions, label=f'Prediction ({model} Model)')

#         # Set the title and labels for the subplot
#         if i == 0:
#             plt.title(f'Average Prediction Performance for {model} Model')
#             plt.legend()
#         if i == 5:
#             plt.xlabel('Timestep')
...
#         # Save the figure
#         plt.savefig(f'Average_Performance_{model}_Model.png')
        


# Create a new figure with 2x3 subplots
fig, axes = plt.subplots(2, 3, figsize=(6, 4), dpi=350)
max_y_values = [30, 40, 10, 2.2, 1.2, 0.6]
# Iterate over each feature
for i, feature in enumerate(features):
    ...
    sns.swarmplot(data=error_values, ax=ax, palette=colors[:3], size=2.5)
    # plot the mean line
    ax.boxplot(error_values, showmeans=False, meanline=False, 
               medianprops={'visible': True, "color": 'red', 'linewidth': 1, 'linestyle': 'dotted'}, 
    ...
    if row == 1 and col == 1:
        ax.set_xlabel('Model')
    if col == 0:
        ax.set_ylabel('MAE [N]' if 'F' in feature else 'MAE [Nm]')
...
# Save the figure
plt.savefig('Model_Error_Boxplots.png')
...
#         # Calculate the average of real_y data over all cycle_indexes
#         avg_error = np.mean(np.abs(real_y_dict[model][:, :, i] - radius_predictions_dict[model][:, :, i]), axis=0)
#         # Calculate the standard deviation of real_y data over all cycle_indexes
#         std_error = np.std(np.abs(real_y_dict[model][:, :, i] - radius_predictions_dict[model][:, :, i]), axis=0)
...
#         if i == 5:
#             plt.xlabel('Timestep')
#         plt.ylabel(f'MAE [N]' if 'F' in feature else f'MAE [Nm]')

#         # Adjust the spacing between subplots
#         plt.tight_layout()

#         # Save the figure
#         plt.savefig(f'Average_Error_{model}_Model.png')
...
#         # Save the figure
#         plt.savefig(f'Average_Performance_{model}_Model_Intensity_05.png')
...
#         # Save the figure
#         plt.savefig(f'Average_Performance_{model}_Model_Intensity_09.png')


errors_mict = {}
mask = np.array(intensity) == 0.5
for model in key_order:   
    errors_mict[model] = error[model][mask]
errors_hiit = {}
mask = np.array(intensity) == 0.9
for model in key_order:   
    errors_hiit[model] = error[model][mask]
...
for i, feature in enumerate(features):
    ...
    # Create a swarmplot for the error values of each model
    sns.swarmplot(data=[errors_mict['TCN'][:,i], errors_hiit['TCN'][:,i]], ax=ax, palette=colors[:2])
    # plot the mean line
    ...
    if col == 0:
        ax.set_ylabel(f'MAE [N]' if 'F' in feature else f'MAE [Nm]')
    if row == 1:
...
# Save the figure
plt.savefig('Error_Comparison_Intensity_Boxplots.png')