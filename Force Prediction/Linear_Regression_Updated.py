...
def get_linear_model():
    linear_model = ElasticNet(alpha=0.003, l1_ratio=0.5, max_iter=2000)
    return linear_model
...
os.chdir('Force Prediction/Linear')
...
# Show the plot
plt.savefig('Boxplot_MDPE_Linear.png')
...
plt.ylabel('Tz [Nm]')
plt.xlim([0, 360])
plt.savefig('Highest_Error_Cycle_Linear.png')
...
plt.savefig('Mean_Cycle_Linear.png')

# Save variables to files
# Save
with open('radius_predictions_Linear.pkl', 'wb') as f:
    pickle.dump(radius_predictions, f)

with open('real_y_Linear.pkl', 'wb') as f:
    pickle.dump(real_y, f)

with open('mdpe_Linear.pkl', 'wb') as f:
    pickle.dump(mdpe, f)

with open('md_column_Linear.pkl', 'wb') as f:
    pickle.dump(md_column, f)

with open('std_column_Linear.pkl', 'wb') as f:
    pickle.dump(std_column, f)

with open('mean_column_Linear.pkl', 'wb') as f:
    pickle.dump(mean_column, f)

## To load models
# loaded_models = []
# for i in range(len(models)):
#     loaded_models.append(tf.keras.models.load_model(f'model_Linear_concat_{i}.h5'))